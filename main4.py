...
def run_tip_adapter_F(cfg, test_features, test_labels, clip_weights, clip_model, train_loader_F, template):
    ...
    for train_idx in range(cfg['train_epoch']): #cfg['train_epoch']
        ...
        for i, (images, target, text) in enumerate(tqdm(train_loader_F)):
            ...
            affinity2 = (0.2 * affinity + 0.8 * image_features ) @ clip_weights.float()
            ...
            cross_logits = (torch.exp(model.alpha)*affinity2.float() @ clip_weights.float())
            ...
            tip_logits = (torch.exp(model.alpha)*affinity2.float() @ clip_weights.float())
...
def main():
    ...
    origin_acc = {}
    for seed in range(3):
        wandb.init(project=("tip-adapter-F"), entity="jingchensun")
    ...
    origin_acc["task"] = "CLIP-Adapter, Crossentropy -D1024-ratio-contrastive-alpha"
